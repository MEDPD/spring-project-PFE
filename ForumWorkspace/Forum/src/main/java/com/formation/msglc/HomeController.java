...
public class HomeController {
	...
	@RequestMapping(value="/updateScore/{id}")
	public String updateScore(@PathVariable(value = "id") Long id, HttpSession session) {
		Auteur auteur = (Auteur) session.getAttribute("auteur");
		
			metier.updateScore(auteur.getId(), id);
			Long sujetId = metier.getReponse(id).getSujet().getId();
			return "redirect:/sujet/"+ sujetId;
