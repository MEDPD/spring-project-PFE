...
public class ForumDaoImpl implements IForumDao{
	...
	public Auteur addAuteur(Auteur auteur) {
		em.persist(auteur);
		auteur.setResponsesIScored(new ArrayList<Reponse>());
		return auteur;
	}
...
///////////////////////////////////////////////////////////////
	...
	public Reponse addReponse(Reponse reponse) {
		...
		// My lines
		reponse.setAuteursScored(new ArrayList<Auteur>());
		reponse.setScore(0);
	...
	// my function
	@Override
	public boolean updateScore(Long auteurId, Long responseId ){
	     Auteur auteur = em.find(Auteur.class, auteurId);
	     if(auteur== null) throw new RuntimeException("Auteur Introvable !!");
	     
	     Reponse reponse = em.find(Reponse.class, responseId);
	     if(reponse== null) throw new RuntimeException("Reponse Introvable !!");
	     
	     for(Reponse resp : auteur.getResponsesIScored()) {
	    	 if(resp.getId() == reponse.getId()) {
	    		 for(Auteur aut : reponse.getAuteursScored()) {
	    			 if(aut.getId() == auteur.getId()) {
	    				 reponse.setScore(reponse.getScore() - 1);
	    				 reponse.getAuteursScored().remove(auteur);
	    				 auteur.getResponsesIScored().remove(reponse);
	    				 return false;
	    			 }
	    		 }
	    		 auteur.getResponsesIScored().add(reponse);
	    		 return  true;
	     ...
	     reponse.setScore(reponse.getScore() + 1);
		 reponse.getAuteursScored().add(auteur);
		 auteur.getResponsesIScored().add(reponse);
	     return true;
	}
